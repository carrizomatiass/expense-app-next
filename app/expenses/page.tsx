...
export default function EnhancedExpenseTracker() {
  ...
  <div className="text-center space-y-2">
    <h1 className="text-2xl sm:text-3xl font-bold">Control de Finanzas</h1>
    <p className="text-sm sm:text-base text-muted-foreground">Gestiona tus ingresos y gastos de manera inteligente</p>
  </div>
  ...
  <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
    ...
    <div className="w-full">
      ...
      <TabsList className="hidden sm:grid w-full grid-cols-4">
        ...
        <TabsTrigger value="add-expense" className="text-xs sm:text-sm">Agregar Gasto</TabsTrigger>
        <TabsTrigger value="add-income" className="text-xs sm:text-sm">Agregar Ingreso</TabsTrigger>
    ...
    <TabsContent value="dashboard" className="space-y-4">
      {expensesByCategory.length > 0 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription className="text-sm">Distribución de tus gastos</CardDescription>
          </CardHeader>
          <CardContent>
              ...
              {expensesByCategory.map((category, index) => (
                <div key={index} className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    <div className="font-medium text-sm sm:text-base">${formatCurrency(category.total)}</div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-lg sm:text-xl">Últimas Transacciones</CardTitle>
          <CardDescription className="text-sm">Tus movimientos más recientes</CardDescription>
        </CardHeader>
        <CardContent>
              ...
              <div key={index} className="flex items-center justify-between p-2 sm:p-3 rounded-lg bg-muted/30">
                <div className="flex items-center space-x-2 flex-1 min-w-0">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-xs text-muted-foreground">
                      <span className="hidden sm:inline">{getCategoryLabel(transaction.category || '')} • </span>
                      {formatDate(transaction.date)}
                    </div>
                  </div>
                </div>
                <div className={`font-medium text-xs sm:text-sm flex-shrink-0 ${transaction.type === 'income' ? 'text-green-600' : 'text-red-600'}`}>
    ...
    <TabsContent value="add-expense">
      <Card>
        <CardHeader>
          <CardTitle className="text-lg sm:text-xl">Agregar Gasto</CardTitle>
          <CardDescription className="text-sm">Registra un nuevo gasto con su categoría</CardDescription>
        </CardHeader>
        <CardContent>
    ...
    <TabsContent value="add-income">
      <Card>
        <CardHeader>
          <CardTitle className="text-lg sm:text-xl">Agregar Ingreso</CardTitle>
          <CardDescription className="text-sm">Registra un nuevo ingreso</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            <Input
    ...
    <TabsContent value="transactions">
      <Card>
        <CardHeader>
          <CardTitle className="text-lg sm:text-xl">Todas las Transacciones</CardTitle>
          <CardDescription className="text-sm">Historial completo con filtros</CardDescription>
        </CardHeader>
        <CardContent>
          {/* Filtros responsivos */}
                ...
                <div className="divide-y max-h-96 overflow-y-auto">
                  {fetching ? (
                    ...
                    filteredTransactions.reverse().map((transaction, idx) => (
                      <div key={idx} className="px-4 py-3 grid grid-cols-5 gap-4 items-center hover:bg-gray-50">
                        <div>
